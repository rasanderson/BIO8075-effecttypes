...
## Manual meta-anlaysis of binary data
In many publications binary data might be reported as 2 x 2 tables, so you would have to reformat them into rows (one row per study) prior to analysis. Again these data are taken from Borenstein page 93 where you can see the fully-worked out examples. The data are in a data.frame called `binary_dat`

```{r binary_show, exercise = TRUE}
binary_dat
```
